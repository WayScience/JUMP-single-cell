...
if not (plates_folder := pathlib.Path("./0.5.quality_control/data/plates")).is_dir():
    plates_folder.mkdir(parents=True)
...
for _, plate_name, plate_s3_path in pd.read_csv(
    ...
    if not (
        plate_folder := pathlib.Path(f"./0.5.quality_control/data/plates/{plate_name}")
    ).is_dir():
    ...
    if not (
        cytotable_output_path := pathlib.Path(
            f"./0.5.quality_control/data/plates/{plate_name}/{plate_name}.parquet"
        )
    ).is_file():
        ...
        cytotable_output_path = cytotable.convert(
            ...
            local_cache_dir="./0.5.quality_control/jump_sqlite_s3_cache/",
            parsl_config=Config(
