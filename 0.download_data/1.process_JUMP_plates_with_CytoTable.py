...
if not (plates_folder := pathlib.Path("./data/plates")).is_dir():
    plates_folder.mkdir()
...
for _, plate_name, plate_s3_path in pd.read_csv(
    ...
    if not (
        cytotable_output_path := pathlib.Path(
            f"./data/plates/{plate_name}/{plate_name}.parquet"
        )
    ).is_file():
        ...
        cytotable_output_path = cytotable.convert(
            ...
            local_cache_dir="./jump_sqlite_s3_cache/",
            parsl_config=Config(
...
shutil.rmtree("./jump_sqlite_s3_cache/")
